...
export function useMcpCatalog() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      toast.error("Failed to load MCP list");
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const updateServer = useCallback(
    async (serverId: number, server_config: Record<string, unknown>) => {
      ...
      try {
        ...
        toast.success(t("library.mcpLibrary.toasts.updated", "Saved successfully"));
        return updated;
      } catch (error) {
  ...
  const createServer = useCallback(
    async (name: string, server_config: Record<string, unknown>) => {
      ...
      try {
        ...
        toast.success(t("library.mcpLibrary.toasts.created", "Created successfully"));
        return created;
      } catch (error) {
  ...
  const uninstallServer = useCallback(
    async (serverId: number, installId: number) => {
      ...
      try {
        ...
        toast.success(
          `${serverName} MCP ${t(
            "library.mcpLibrary.toasts.uninstalled",
            "uninstalled",
          )}`,
        );
      } catch (error) {
